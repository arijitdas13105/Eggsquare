...
const Checkout = () => {
  ...
  const handlePayment = async () => {
    try {
      console.log("sendOrderData at checkout-235");
      setIsLoading(true); 
      const customerId = user.customer._id;
      ...
      console.log("sendOrderData at checkout-244",orderData);

      const options = {
        key: process.env.KEY_SECRET,
        // key: "kYNvwDsfM7z5CuvK6GyyaBY7",
        amount: orderData.amount,
        currency: orderData.currency,
        name: "Eggs Square",
        description: "Payment for Your Order",
        ...
        handler: async (response) => {
          try {
            setIsLoading(false)
            console.log("response at checkout-251",response);
            const verifyResponse = await axios.post(
              `${BASE_URL}/api/payments/verify`,
              response
            );
            console.log("verifyResponse at checkout-263",verifyResponse);
            const verificationData = verifyResponse.data;
            console.log("verificationData at checkout-265",verificationData);
            if (verificationData.message === "Payment verified successfully") {
              //cartItemData need to get "title,price,...total" nor cartItems providing "id" also which i dont need
              const cartItemData = cartItems.map((item) => ({
                title: item.title,
              ...
              console.log("sendOrderData at checkout-283",sendOrderData);
              dispatch(placeOrder(user.customer._id, sendOrderData));
